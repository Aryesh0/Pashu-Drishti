...
import { AMR_DRUG_LIBRARY, getDrugProfile, getWithdrawalSummary } from '../../utils/livestockCatalog'
...
export default function AmrPage() {
  ...
  const selectedAnimalSpecies = animals.find((animal) => form.animalIds.includes(animal.id))?.species || animals[0]?.species || 'COW'
  ...
  const updateDrug = (index, key, value) => {
    setForm((current) => {
      const nextDrugs = current.drugsUsed.map((drug, i) => {
        if (i !== index) return drug
        if (key === 'catalogKey') {
          const profile = getDrugProfile(value, selectedAnimalSpecies)
          return profile
            ? {
                ...drug,
                catalogKey: value,
                ...
                routeOfAdministration: profile.routeOfAdministration,
                durationDays: profile.durationDays,
                milkWithdrawalDays: profile.milkWithdrawalDays,
                meatWithdrawalDays: profile.meatWithdrawalDays,
                criticallyImportantAntibiotic: profile.criticallyImportantAntibiotic,
              }
            : blankDrug()
        }
        return { ...drug, [key]: value }
      })

      const maxDuration = Math.max(0, ...nextDrugs.map((drug) => Number(drug.durationDays) || 0))
      const maxMilk = Math.max(0, ...nextDrugs.map((drug) => Number(drug.milkWithdrawalDays) || 0))
      const maxMeat = Math.max(0, ...nextDrugs.map((drug) => Number(drug.meatWithdrawalDays) || 0))
      const treatmentEndDate = current.treatmentStartDate && maxDuration
        ? addDays(current.treatmentStartDate, Math.max(maxDuration - 1, 0))
        : current.treatmentEndDate
      return {
        ...current,
        drugsUsed: nextDrugs,
        treatmentEndDate,
        milkWithdrawalEndDate: treatmentEndDate && maxMilk ? addDays(treatmentEndDate, maxMilk) : current.milkWithdrawalEndDate,
        meatWithdrawalEndDate: treatmentEndDate && maxMeat ? addDays(treatmentEndDate, maxMeat) : current.meatWithdrawalEndDate,
      }
    })
  }

  const addDrug = () => {
    setForm((current) => ({ ...current, drugsUsed: [...current.drugsUsed, blankDrug()] }))
  }

  const removeDrug = (index) => {
    setForm((current) => ({ ...current, drugsUsed: current.drugsUsed.filter((_, i) => i !== index) }))
  }

  useEffect(() => {
    setForm((current) => {
      const nextDrugs = current.drugsUsed.map((drug) => {
        if (!drug.catalogKey) return drug
        const profile = getDrugProfile(drug.catalogKey, selectedAnimalSpecies)
        return profile ? {
          ...drug,
          milkWithdrawalDays: profile.milkWithdrawalDays,
          meatWithdrawalDays: profile.meatWithdrawalDays,
  ...
  }, [selectedAnimalSpecies, form.treatmentStartDate])
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            {animals.length > 0 && (
              <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4">
                <p className="font-body text-sm font-semibold text-blue-800">Withdrawal guidance base</p>
                <p className="font-body text-xs text-blue-700 mt-1">
                  Automatic AMR withdrawal defaults are being calculated for {humanizeEnum(selectedAnimalSpecies)} based on the selected animals and drugs.
                </p>
              </div>
            )}

            <div className="flex items-center justify-between">
              <p className="font-display text-lg font-semibold text-earth-900">Drug Entries</p>
              <button type="button" onClick={addDrug} className="btn-secondary text-sm">Add Drug</button>
            </div>

            {form.drugsUsed.map((drug, index) => (
              <div key={`drug-${index}`} className="rounded-2xl border border-earth-200 p-4">
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <Field label="Drug Preset">
                    <select className="input-field" value={drug.catalogKey} onChange={(event) => updateDrug(index, 'catalogKey', event.target.value)}>
                      <option value="">Custom drug</option>
                      {AMR_DRUG_LIBRARY.map((item) => <option key={item.key} value={item.key}>{item.drugName}</option>)}
                    </select>
                  </Field>
                ...
                <p className="mt-3 text-xs font-body text-earth-500">
                  {getWithdrawalSummary(Number(drug.milkWithdrawalDays) || 0, Number(drug.meatWithdrawalDays) || 0)}
                </p>
                {form.drugsUsed.length > 1 && (
                  <button type="button" onClick={() => removeDrug(index)} className="mt-3 text-xs font-body text-red-600 hover:underline">
                    Remove drug
                  </button>
                )}
              </div>
