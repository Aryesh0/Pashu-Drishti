...
export default function ProducerDashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const animalResponses = await Promise.all(myFarms.map((farm) => animalAPI.getByFarm(farm.id, 0, 100)))
      const mrlResponses = await Promise.all(myFarms.map((farm) => mrlAPI.getByFarm(farm.id, 0, 100)))
      const amrResponses = await Promise.all(myFarms.map((farm) => amrAPI.getByFarm(farm.id, 0, 100)))
