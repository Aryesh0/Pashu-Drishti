        ...
        <Modal open={vaccinationOpen} onClose={() => setVaccinationOpen(false)} title="Record Vaccination">
          <form onSubmit={handleVaccinationSubmit} className="space-y-4">
            <Field label="Vaccine Name">
              <input className="input-field" value={vaccinationForm.vaccineName} onChange={(event) => setVaccinationForm((current) => ({ ...current, vaccineName: event.target.value }))} required />
            </Field>
            <Field label="Disease">
              <input className="input-field" value={vaccinationForm.disease} onChange={(event) => setVaccinationForm((current) => ({ ...current, disease: event.target.value }))} />
            </Field>
            <div className="grid sm:grid-cols-2 gap-4">
              <Field label="Vaccination Date">
                <input type="date" className="input-field" value={vaccinationForm.vaccinationDate} onChange={(event) => setVaccinationForm((current) => ({ ...current, vaccinationDate: event.target.value }))} required />
              </Field>
              <Field label="Next Due Date">
                <input type="date" className="input-field" value={vaccinationForm.nextDueDate} onChange={(event) => setVaccinationForm((current) => ({ ...current, nextDueDate: event.target.value }))} />
              </Field>
              <Field label="Administered By">
                <input className="input-field" value={vaccinationForm.administeredBy} onChange={(event) => setVaccinationForm((current) => ({ ...current, administeredBy: event.target.value }))} />
              </Field>
              <Field label="Batch Number">
                <input className="input-field" value={vaccinationForm.batchNumber} onChange={(event) => setVaccinationForm((current) => ({ ...current, batchNumber: event.target.value }))} />
              </Field>
            </div>
            <div className="flex gap-3">
